...
public class AdminController {
    ...
    //트레이너 신청 목록 리스트 보기
    @GetMapping("/get-trainers-save-form/list")
    public ResponseEntity<List<TrainerSaveFormDTO>> getTrainersSaveForm(@AuthenticationPrincipal UserDetails adminDetails) {
...
//좋아요는 사용자들이 나중에 추가함
    ...
    public ResponseEntity<String> updateGym(@PathVariable Long gymId,
                                            @RequestBody CreateGymDTO createGymDTO,
                                            @AuthenticationPrincipal UserDetails adminDetails) {
...
/**admin 기능
 * 권한 변경 완료
 * 트레이너 신청한 회원 정보 조회 TrainerSaveRequestDTO 를 리스트로 조회
 * 헬스장 등록 , 삭제 , 수정
 **/
}
